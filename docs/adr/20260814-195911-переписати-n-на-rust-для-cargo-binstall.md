...
## Update 2026-08-14: бінарник і crate перейменовано `n` → `g`

Користувач попросив перейменувати бінарник на `g` (скорочено від git, за аналогією з
`ripgrep`/`rg`). Разом з бінарником перейменовано й package/crate — узгоджено з
патерном `n7n-git` (`n7n-<binary>`). `g7n` теж вільне на crates.io (перевірено), але
обрано **`n7n-g`** — узгоджено з попереднім `n7n-git`.

Перейменовано: `Cargo.toml` (`name = "n7n-g"`, `[lib] name = "n7n_g"`,
`[[bin]] name = "g"`, `pkg-url` → `g-{ target }`), `src/bin/n.rs` → `src/bin/g.rs`,
усі doc-коментарі модулів (`n getw` → `g getw` тощо), `README.md`,
`.forgejo/workflows/release.yml` (asset-імена `g-*`, `Formula/g.rb`, `class G`,
`bin.install "..." => "g"`).

**Не перейменовано**: внутрішній тип помилки `NError` лишається як є (не звʼязаний з
назвою бінарника).

## Update 2026-08-15: репозиторій перейменовано `7n/n` → `7n/g`

Користувач попросив перейменувати й сам репозиторій на Forgejo, узгоджено з новою
назвою бінарника — на відміну від початкового рішення в попередньому update
("репо ≠ бінарник, як у `mt-rust`"). Виконано через `tea repos edit --repo 7n/n
--name g` (Forgejo API-доступу для прямих HTTP-викликів немає — `tea` вже мав
авторизовану сесію на `git.7n.ai`). Новий адрес:
[git.7n.ai/7n/g](https://git.7n.ai/7n/g).

Оновлено всі літеральні посилання на URL: `Cargo.toml` (`repository`), `README.md`,
`.forgejo/workflows/release.yml` (`homepage`/`url` у Homebrew-формулі — `REPO` там
береться з `github.repository`-контексту динамічно, тому не потребував правки).
Локальний git remote (`origin`) теж переведено на нову URL.
