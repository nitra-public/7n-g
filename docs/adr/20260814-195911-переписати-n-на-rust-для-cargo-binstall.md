...
**Не перейменовано**: назва репозиторію лишається `git.7n.ai/7n/n` (репо ≠ бінарник,
як і в `mt-rust`, де репо називається `mt-rust`, а бінарник — `mt`); внутрішній тип
помилки `NError` теж лишається як є (не звʼязаний з назвою бінарника).
